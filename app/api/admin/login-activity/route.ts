import { NextResponse } from "next/server";
import { requireSiteAdmin } from "@/lib/auth/isSiteAdmin";
import { fetchPlatformLoginActivity } from "@/lib/admin/platformData";
import { isSupabaseAdminConfigured } from "@/lib/supabase/admin";

export async function GET(request: Request) {
  if (!isSupabaseAdminConfigured()) {
    return NextResponse.json({ error: "Admin API is not configured on the server." }, { status: 503 });
  }

  const admin = await requireSiteAdmin();
  if (!admin) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }

  const { searchParams } = new URL(request.url);
  const limit = Math.min(500, Math.max(10, Number(searchParams.get("limit") ?? 150)));

  try {
    const events = await fetchPlatformLoginActivity(limit);
    return NextResponse.json({ ok: true, events });
  } catch (e) {
    const message = e instanceof Error ? e.message : "Failed to load login activity";
    return NextResponse.json({ error: message }, { status: 500 });
  }
}