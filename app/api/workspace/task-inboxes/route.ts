...
import {
  createWorkspaceTaskInbox,
  listWorkspaceTaskInboxes,
...
export async function GET(request: Request) {
  ...
  try {
    const inboxes = await listWorkspaceTaskInboxes(workspaceId, user.id);
...
export async function POST(request: Request) {
  ...
  try {
    const inbox = await createWorkspaceTaskInbox({
      workspaceId,
      userId: user.id,
    });
