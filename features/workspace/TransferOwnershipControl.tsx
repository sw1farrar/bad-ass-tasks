...
export function TransferOwnershipControl({
  ...
  const selectControl = (
    <select
      ...
      className="bg-[#111114] border border-white/20 rounded-lg px-2.5 py-1.5 text-xs focus:outline-none focus:border-[#c084fc] max-w-[9.5rem] truncate"
  ...
  return (
    <>
      <div className="flex flex-col items-end gap-2 shrink-0 max-w-[15rem]">
        <span
          className="text-[10px] text-[#71717a] text-right leading-snug"
          title="Transfer ownership before you can leave this workspace"
        >
          Transfer ownership to leave
        </span>
        {candidates.length === 0 ? (
          <span className="text-[10px] text-[#71717a] text-right">Invite a teammate first</span>
        ) : (
