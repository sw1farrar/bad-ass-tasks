...
export function StartNextMeetingModal({
  ...
  return (
    <ConfirmationModal
      ...
      description="A new meeting will be created with selected carry-over topics."
      details={
        <div className="space-y-2 text-sm text-text-muted">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            Include {continuedCount} continued topic{continuedCount === 1 ? "" : "s"}
          </label>
          {openCount > 0 && (
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="rounded border-border-glass"
              />
              Include {openCount} open topic{openCount === 1 ? "" : "s"}
            </label>
          )}
        </div>
      }
