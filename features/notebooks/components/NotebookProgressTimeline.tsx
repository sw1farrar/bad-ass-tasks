...
interface NotebookProgressTimelineProps {
...
export function NotebookProgressTimeline({
  entries,
  members,
  currentUserId,
  emptyMessage = "No progress notes yet. Add one below — each entry is timestamped automatically.",
  onUpdateEntry,
  onRequestDeleteEntry,
}: NotebookProgressTimelineProps) {
  const [editingId, setEditingId] = useState<string | null>(null);
  const [draftBody, setDraftBody] = useState("");
  const [isSaving, setIsSaving] = useState(false);

  const sortedEntries = useMemo(
    () => sortProgressEntriesNewestFirst(entries),
    [entries],
  );
  ...
  if (sortedEntries.length === 0) {
    return (
      <p className="text-sm text-text-muted px-4 py-6 text-center">{emptyMessage}</p>
    );
  }
