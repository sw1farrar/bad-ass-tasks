...
function buildDualAuthHtml(params: DualAuthEmailParams): string {
...
<head>
  ...
  <title>Sign-in verification — Badazz Tasks</title>
</head>
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background-color:#111114;border:1px solid rgba(255,255,255,0.08);border-radius:20px;overflow:hidden;">
          <tr>
            <td style="padding:32px 32px 24px;text-align:center;background:linear-gradient(180deg,rgba(192,132,252,0.14) 0%,rgba(17,17,20,0) 100%);">
              ...
              <h1 style="margin:0;font-size:24px;font-weight:600;letter-spacing:-0.02em;color:#f4f4f5;line-height:1.2;">Verify your sign-in</h1>
              <p style="margin:12px 0 0;font-size:14px;line-height:1.6;color:#a1a1aa;">Enter this code to finish signing in. We sent it to your registered email for extra security.</p>
            </td>
          </tr>
          <tr>
            <td style="padding:8px 32px 28px;text-align:center;">
              ...
              <p style="margin:20px 0 0;font-size:13px;line-height:1.6;color:#71717a;">This code expires in 10 minutes. If you didn&apos;t try to sign in, change your password immediately.</p>
            </td>
          </tr>
          <tr>
...
export async function sendDualAuthEmail(
  ...
  const subject = `${params.code} is your Badazz Tasks sign-in code`;
  ...
  const response = await fetch("https://api.brevo.com/v3/smtp/email", {
    ...
    body: JSON.stringify({
      ...
      to: [{ email: params.to }],
      subject,
      htmlContent: buildDualAuthHtml(params),
  ...
  if (!response.ok) {
    ...
    console.error("[brevo] dual auth email failed", response.status, detail);
